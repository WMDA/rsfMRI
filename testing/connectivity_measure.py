...
def cylic_connectivity(time_series):
    """ Returns the lead lag matrix from a given time series

    Parameters
    ----------
    time_series : 2D numpy.ndarray
        The raw time series 

    Returns
    -------
    Lead lag matrix : 2D numpy.ndarray
        Lead Lag matrix.
...
class ConnectivityMeasure(BaseEstimator, TransformerMixin):
    ...
    kind : {"covariance", "correlation", "partial correlation",\
            "tangent", "precision", "cyclic"}, default='covariance'
        The matrix kind.
    ...
    def _fit_transform(
        ...
        if self.kind == "correlation":
            ...
            if self.kind in ("covariance", "tangent"):
                ...
                connectivities = [cylic_connectivity(x) for x in X]
            else:
                allowed_kinds = (
                    "correlation",
                    "partial correlation",
